...
public class AlgoEFIM {
	...
	public static int timeProject = 0;
	...
	public void runAlgorithm(int minUtil, String inputPath, String outputPath, String TaxonomyPath,
		 ...
		 * condition List<Integer> itemsToExplore = new ArrayList<Integer>(); // if
		 * subtree utility pruning is activated for (Integer item : itemsToKeep) { // if
		 * the subtree utility is higher or equal to minutil, then keep it if
		 * (utilityBinArraySU[item] >= minUtil) { itemsToExplore.add(item); } }
		 */
	...
	public void useUtilityBinArrayToCalculateSubtreeUtilityFirstTime(Dataset dataset) {
		...
		for (Transaction transaction : dataset.getTransactions()) {
			...
			for (int itemParent : transaction.parentsInTransaction.keySet()) {
				...
				utilityBinArraySU[itemParent] += transaction.parentsInTransaction.get(itemParent);
			}
		}
	}
	...
	private void useUtilityBinArraysToCalculateUpperBounds(List<Transaction> transactionsPe, int j,
		...
		for (Transaction transaction : transactionsPe) {
			...
			for (Integer itemParent : transaction.parentsInTransaction.keySet()) {
				...
				utilityBinArraySU[itemParent] += sumU + transaction.prefixUtility
						+ transaction.parentsInTransaction.get(itemParent);
				utilityBinArrayLU[itemParent] += transaction.transactionUtility + transaction.prefixUtility;
